-- H-VOL1 readiness hardening.
-- Readiness-only: no profitability scan, no OOS outcome, no cutoff selection.
-- The predecessor used row-based windows without a continuity gate. This version
-- proves the 120-row predecessor spans the exact 120 real minutes and refuses
-- READY when the covered BTCUSDT 1m dataset contains any detected gap.
...
WITH base AS (
  SELECT o.open_time_ms,o.high,o.low,o.close,o.volume,
         ...
         lag(o.open_time_ms,120) OVER (ORDER BY o.open_time_ms) AS prior_first_ms,
         lag(o.open_time_ms,1) OVER (ORDER BY o.open_time_ms) AS prior_last_ms,
  ...
  SELECT b.*,lag(b.open_time_ms) OVER (ORDER BY b.open_time_ms) AS prev_ms FROM base b
), bounds AS (
  SELECT min(open_time_ms)::bigint AS start_ms,max(open_time_ms)::bigint AS end_ms,
         count(*)::bigint AS candles,
         count(*) FILTER (WHERE prev_ms IS NOT NULL AND open_time_ms-prev_ms<>60000)::bigint AS gap_count
  FROM ordered
), coverage AS (
  ...
  SELECT b.open_time_ms FROM base b
  WHERE b.prior_first_ms=b.open_time_ms-120*60000
    AND b.prior_last_ms=b.open_time_ms-60000
), candidates AS (
  SELECT b.open_time_ms AS trigger_ms FROM base b CROSS JOIN thresholds t
  WHERE p_oos_start_ms IS NOT NULL AND b.open_time_ms>=p_oos_start_ms AND b.open_time_ms<p_as_of_ms
    AND b.prior_first_ms=b.open_time_ms-120*60000
    AND b.prior_last_ms=b.open_time_ms-60000
  ...
  SELECT count(*) FILTER (WHERE EXISTS (SELECT 1 FROM base e WHERE e.open_time_ms=c.trigger_ms+60000))::bigint AS entry_count,
         count(*) FILTER (WHERE EXISTS (SELECT 1 FROM base e WHERE e.open_time_ms=c.trigger_ms+60000)
             AND EXISTS (SELECT 1 FROM base x WHERE x.open_time_ms=c.trigger_ms+121*60000))::bigint AS exit_count
  FROM candidates c
), metrics AS (
  SELECT c.*,c.gap_count=0 AS continuity_ok,
    p_oos_start_ms IS NOT NULL AND p_oos_start_ms<p_as_of_ms AS boundary_frozen,
    coalesce((SELECT count(*) FROM range_windows),0)::bigint AS range_count,
    coalesce((SELECT count(*) FROM ratios),0)::bigint AS ratio_count,
    coalesce((SELECT count(*) FROM ratios r WHERE p_oos_start_ms IS NOT NULL AND r.open_time_ms<p_oos_start_ms),0)::bigint AS train_count,
    coalesce((SELECT count(*) FROM ratios r WHERE p_oos_start_ms IS NOT NULL AND r.open_time_ms>=p_oos_start_ms),0)::bigint AS oos_count,
    t.p90,t.p10,a.entry_count,a.exit_count
  FROM coverage c CROSS JOIN thresholds t CROSS JOIN availability a
)
SELECT 'H-VOL1',
  CASE WHEN m.boundary_frozen AND m.continuity_ok AND m.range_count>0 AND m.ratio_count>0
  ...
  greatest(0,m.expected-m.candles),m.continuity_ok,m.range_count,m.ratio_count,
