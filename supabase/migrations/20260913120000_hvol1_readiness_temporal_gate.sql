...
--
-- The prior readiness implementation used row-based windows and did not include
-- continuity_ok in the READY gate. This version keeps the bounded window
-- computation but proves each 120-row predecessor spans exactly 120 real minutes,
-- and refuses READY when the covered BTCUSDT 1m dataset itself contains gaps.
...
WITH base AS (
  ...
  SELECT b.*,lag(b.open_time_ms) OVER (ORDER BY b.open_time_ms) AS prev_ms
  FROM base b
), bounds AS (
  ...
  FROM ratios
  WHERE p_oos_start_ms IS NOT NULL AND open_time_ms<p_oos_start_ms
), range_windows AS (
  SELECT b.open_time_ms
  FROM base b
...
SELECT 'H-VOL1',
  ...
  greatest(0,m.expected-m.candles),m.continuity_ok,m.range_count,
