-- H-VOL1 readiness fast path. No outcome scan and no OOS boundary selection.
-- The dashboard calls readiness without a boundary; avoid expensive breakout-window
-- evaluation when there is no training/OOS split to evaluate.
...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT min(open_time_ms)::bigint AS start_ms,max(open_time_ms)::bigint AS end_ms,count(*)::bigint AS candles FROM base
  ), coverage AS (
    SELECT b.*,greatest(0,((b.end_ms-b.start_ms)/60000)+1)::bigint AS expected FROM bounds b
  ), ratios AS (SELECT open_time_ms,taker_ratio FROM base WHERE taker_ratio IS NOT NULL), thresholds AS (
    ...
    WHERE b.open_time_ms>=p_oos_start_ms AND b.open_time_ms<p_as_of_ms
      AND b.prior_start_ms=b.open_time_ms-120*60000 AND b.taker_ratio IS NOT NULL
      AND ((b.close>b.prior_high AND b.taker_ratio>=t.p90) OR (b.close<b.prior_low AND b.taker_ratio<=t.p10))
  ), availability AS (
    SELECT count(*) FILTER (WHERE EXISTS (SELECT 1 FROM base e WHERE e.open_time_ms=c.trigger_ms+60000))::bigint AS entry_count,
      count(*) FILTER (WHERE EXISTS (SELECT 1 FROM base e WHERE e.open_time_ms=c.trigger_ms+60000) AND EXISTS (SELECT 1 FROM base x WHERE x.open_time_ms=c.trigger_ms+121*60000))::bigint AS exit_count
    FROM candidates c
  ), metrics AS (
  ...
  SELECT 'H-VOL1',CASE WHEN m.boundary_frozen AND m.range_count>0 AND m.ratio_count>0 AND m.p90 IS NOT NULL AND m.p10 IS NOT NULL AND m.entry_count>0 AND m.exit_count>0 THEN 'READY_FOR_INDEPENDENT_AUDIT' ELSE 'NOT_READY' END,
    p_as_of_ms,p_oos_start_ms,m.start_ms,m.end_ms,m.candles,m.expected,greatest(0,m.expected-m.candles),m.expected=m.candles,m.range_count,m.ratio_count,m.train_count,m.oos_count,m.p90,m.p10,m.entry_count,m.exit_count,
    'GREEDY_CHRONOLOGICAL_PREVIOUS_ACCEPTED_EXIT',10::numeric,12::numeric,m.boundary_frozen,false,'NOT GRANTED'
  FROM metrics m;
END;
$function$;
